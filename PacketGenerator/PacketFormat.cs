...
{0}
    ...
    void MakePacket<T>(PacketSession packetSession, ArraySegment<byte> buffer) where T : IPacket, new()
    {{
        T packet = new T();
        packet.Read(buffer);
        ...
        if( _handler.TryGetValue(packet.Protocol, out action))
            action.Invoke(packetSession, packet);
    }}
}}
";
...
@"this.{0} = ({1})s.Array[s.Offset + count];
count += sizeof({1});";
...
count += {0}Len;";
    ...
    // {0} 리스트 이름 [대문자]
    // {1} 리스트 이름 [소문자]
    public static string readListStringFormat = 
@"this.{1}s.Clear();
...
count += {0}Len;";
    ...
    // {0} 리스트 이름 [대문자]
    // {1} 리스트 이름 [소문자]
    public static string writeListStringFormat = 
